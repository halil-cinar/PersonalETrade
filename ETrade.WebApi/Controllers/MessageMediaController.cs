...
namespace ETrade.WebApi.Controllers
{
    ...
    public class MessageMediaController : ControllerBase
    {
        ...
        private readonly IMessageMediaService _messagemediaManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public MessageMediaController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _messagemediaManager = new MessageMediaManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<MessageMediaLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<MessageMediaFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<MessageMediaLoadMoreDto>();
            try
            {

                var result = _messagemediaManager.FilterMessageMediaList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.MessageMediaExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<MessageMediaListDto> Add([FromBody] MessageMediaDto messagemediaDto, [FromQuery] string Authorization)
        {
            var response = new Response<MessageMediaListDto>();
            try
            {
                var result = _messagemediaManager.AddMessageMedia(messagemediaDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<MessageMediaListDto> Update([FromBody] MessageMediaDto messagemediaDto, [FromQuery] string Authorization)
        {
            var response = new Response<MessageMediaListDto>();
            try
            {
                var result = _messagemediaManager.UpdateMessageMedia(messagemediaDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<MessageMediaListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<MessageMediaListDto>();
            try
            {
                var result = _messagemediaManager.DeleteMessageMedia(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<MessageMediaListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<MessageMediaListDto>();
            try
            {
                var result = _messagemediaManager.GetMessageMedia(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
