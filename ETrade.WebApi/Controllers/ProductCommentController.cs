...
namespace ETrade.WebApi.Controllers
{
    ...
    public class ProductCommentController : ControllerBase
    {
        ...
        public ProductCommentController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _productcommentManager = new ProductCommentManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<ProductCommentLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<ProductCommentFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<ProductCommentLoadMoreDto>();
            try
            {

                var result = _productcommentManager.FilterProductCommentList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.ProductCommentExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<ProductCommentListDto> Add([FromBody] ProductCommentDto productcommentDto, [FromQuery] string Authorization)
        {
            var response = new Response<ProductCommentListDto>();
            try
            {
                var result = _productcommentManager.AddProductComment(productcommentDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<ProductCommentListDto> Update([FromBody] ProductCommentDto productcommentDto, [FromQuery] string Authorization)
        {
            var response = new Response<ProductCommentListDto>();
            try
            {
                var result = _productcommentManager.UpdateProductComment(productcommentDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<ProductCommentListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ProductCommentListDto>();
            try
            {
                var result = _productcommentManager.DeleteProductComment(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<ProductCommentListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ProductCommentListDto>();
            try
            {
                var result = _productcommentManager.GetProductComment(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
