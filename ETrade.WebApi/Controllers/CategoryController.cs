...
namespace ETrade.WebApi.Controllers
{
    ...
    public class CategoryController : ControllerBase
    {
        ...
        public CategoryController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _categoryManager = new CategoryManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<CategoryLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<CategoryFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<CategoryLoadMoreDto>();
            try
            {

                var result = _categoryManager.FilterCategoryList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.CategoryExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<CategoryListDto> Add([FromBody] CategoryDto categoryDto, [FromQuery] string Authorization)
        {
            var response = new Response<CategoryListDto>();
            try
            {
        ...
        [HttpPut]
        [Route("Update")]
        public Response<CategoryListDto> Update([FromBody] CategoryDto categoryDto, [FromQuery] string Authorization)
        {
            var response = new Response<CategoryListDto>();
            try
            {
                var result = _categoryManager.UpdateCategory(categoryDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<CategoryListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CategoryListDto>();
            try
            {
                var result = _categoryManager.DeleteCategory(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<CategoryListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CategoryListDto>();
            try
            {
                var result = _categoryManager.GetCategory(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
