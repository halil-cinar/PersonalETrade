...
namespace ETrade.WebApi.Controllers
{
    ...
    public class RoleController : ControllerBase
    {
        ...
        public RoleController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _roleManager = new RoleManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<RoleLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<RoleFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<RoleLoadMoreDto>();
            try
            {

                var result = _roleManager.FilterRoleList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.RoleExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<RoleListDto> Add([FromBody] RoleDto roleDto, [FromQuery] string Authorization)
        {
            var response = new Response<RoleListDto>();
            try
            {
                var result = _roleManager.AddRole(roleDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<RoleListDto> Update([FromBody] RoleDto roleDto, [FromQuery] string Authorization)
        {
            var response = new Response<RoleListDto>();
            try
            {
                var result = _roleManager.UpdateRole(roleDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<RoleListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<RoleListDto>();
            try
            {
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<RoleListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<RoleListDto>();
            try
            {
                var result = _roleManager.GetRole(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
