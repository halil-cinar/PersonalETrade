...
namespace ETrade.WebApi.Controllers
{
    ...
    public class CommentController : ControllerBase
    {
        ...
        public CommentController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _commentManager = new CommentManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<CommentLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<CommentFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<CommentLoadMoreDto>();
            try
            {

                var result = _commentManager.FilterCommentList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.CommentExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<CommentListDto> Add([FromBody] CommentDto commentDto, [FromQuery] string Authorization)
        {
            var response = new Response<CommentListDto>();
            try
            {
                var result = _commentManager.AddComment(commentDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<CommentListDto> Update([FromBody] CommentDto commentDto, [FromQuery] string Authorization)
        {
            var response = new Response<CommentListDto>();
            try
            {
                var result = _commentManager.UpdateComment(commentDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<CommentListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CommentListDto>();
            try
            {
                var result = _commentManager.DeleteComment(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<CommentListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CommentListDto>();
            try
            {
                var result = _commentManager.GetComment(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
