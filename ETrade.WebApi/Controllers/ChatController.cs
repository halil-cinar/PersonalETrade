...
namespace ETrade.WebApi.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        private readonly IChatService _chatManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public ChatController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _chatManager = new ChatManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<ChatLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<ChatFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<ChatLoadMoreDto>();
            try
            {

                var result = _chatManager.FilterChatList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.ChatExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<ChatListDto> Add([FromBody] ChatDto chatDto, [FromQuery] string Authorization)
        {
            var response = new Response<ChatListDto>();
            try
            {
                var result = _chatManager.AddChat(chatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<ChatListDto> Update([FromBody] ChatDto chatDto, [FromQuery] string Authorization)
        {
            var response = new Response<ChatListDto>();
            try
            {
                var result = _chatManager.UpdateChat(chatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<ChatListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ChatListDto>();
            try
            {
                var result = _chatManager.DeleteChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<ChatListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ChatListDto>();
            try
            {
                var result = _chatManager.GetChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
