...
namespace ETrade.WebApi.Controllers
{
    ...
    public class UserChatController : ControllerBase
    {
        ...
        private readonly IUserChatService _userchatManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public UserChatController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _userchatManager = new UserChatManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<UserChatLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<UserChatFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<UserChatLoadMoreDto>();
            try
            {

                var result = _userchatManager.FilterUserChatList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.UserChatExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<UserChatListDto> Add([FromBody] UserChatDto userchatDto, [FromQuery] string Authorization)
        {
            var response = new Response<UserChatListDto>();
            try
            {
                var result = _userchatManager.AddUserChat(userchatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<UserChatListDto> Update([FromBody] UserChatDto userchatDto, [FromQuery] string Authorization)
        {
            var response = new Response<UserChatListDto>();
            try
            {
                var result = _userchatManager.UpdateUserChat(userchatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<UserChatListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<UserChatListDto>();
            try
            {
                var result = _userchatManager.DeleteUserChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<UserChatListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<UserChatListDto>();
            try
            {
                var result = _userchatManager.GetUserChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
