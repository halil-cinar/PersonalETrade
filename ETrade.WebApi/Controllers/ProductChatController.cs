...
namespace ETrade.WebApi.Controllers
{
    ...
    public class ProductChatController : ControllerBase
    {
        ...
        private readonly IProductChatService _productchatManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public ProductChatController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _productchatManager = new ProductChatManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<ProductChatLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<ProductChatFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<ProductChatLoadMoreDto>();
            try
            {

                var result = _productchatManager.FilterProductChatList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.ProductChatExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<ProductChatListDto> Add([FromBody] ProductChatDto productchatDto, [FromQuery] string Authorization)
        {
            var response = new Response<ProductChatListDto>();
            try
            {
                var result = _productchatManager.AddProductChat(productchatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<ProductChatListDto> Update([FromBody] ProductChatDto productchatDto, [FromQuery] string Authorization)
        {
            var response = new Response<ProductChatListDto>();
            try
            {
                var result = _productchatManager.UpdateProductChat(productchatDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<ProductChatListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ProductChatListDto>();
            try
            {
                var result = _productchatManager.DeleteProductChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<ProductChatListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<ProductChatListDto>();
            try
            {
                var result = _productchatManager.GetProductChat(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
