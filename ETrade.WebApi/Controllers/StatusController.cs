...
namespace ETrade.WebApi.Controllers
{
    ...
    public class StatusController : ControllerBase
    {
        ...
        private readonly IStatusService _statusManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public StatusController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _statusManager = new StatusManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<StatusLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<StatusFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<StatusLoadMoreDto>();
            try
            {

                var result = _statusManager.FilterStatusList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.StatusExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<StatusListDto> Add([FromBody] StatusDto statusDto, [FromQuery] string Authorization)
        {
            var response = new Response<StatusListDto>();
            try
            {
                var result = _statusManager.AddStatus(statusDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<StatusListDto> Update([FromBody] StatusDto statusDto, [FromQuery] string Authorization)
        {
            var response = new Response<StatusListDto>();
            try
            {
                var result = _statusManager.UpdateStatus(statusDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<StatusListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<StatusListDto>();
            try
            {
                var result = _statusManager.DeleteStatus(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<StatusListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<StatusListDto>();
            try
            {
                var result = _statusManager.GetStatus(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
