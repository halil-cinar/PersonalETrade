...
namespace ETrade.WebApi.Controllers
{
    ...
    public class OrderController : ControllerBase
    {
        ...
        public OrderController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _orderManager = new OrderManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<OrderLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<OrderFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<OrderLoadMoreDto>();
            try
            {

                var result = _orderManager.FilterOrderList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.OrderExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<OrderListDto> Add([FromBody] OrderDto orderDto, [FromQuery] string Authorization)
        {
            var response = new Response<OrderListDto>();
            try
            {
                var result = _orderManager.AddOrder(orderDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<OrderListDto> Update([FromBody] OrderDto orderDto, [FromQuery] string Authorization)
        {
            var response = new Response<OrderListDto>();
            try
            {
        ...
        public Response<OrderListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<OrderListDto>();
            try
            {
                var result = _orderManager.DeleteOrder(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<OrderListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<OrderListDto>();
            try
            {
                var result = _orderManager.GetOrder(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
