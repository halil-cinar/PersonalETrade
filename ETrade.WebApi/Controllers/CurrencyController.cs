...
namespace ETrade.WebApi.Controllers
{
    ...
    public class CurrencyController : ControllerBase
    {
        ...
        public CurrencyController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _currencyManager = new CurrencyManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<CurrencyLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<CurrencyFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<CurrencyLoadMoreDto>();
            try
            {

                var result = _currencyManager.FilterCurrencyList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.CurrencyExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<CurrencyListDto> Add([FromBody] CurrencyDto currencyDto, [FromQuery] string Authorization)
        {
            var response = new Response<CurrencyListDto>();
            try
            {
                var result = _currencyManager.AddCurrency(currencyDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<CurrencyListDto> Update([FromBody] CurrencyDto currencyDto, [FromQuery] string Authorization)
        {
            var response = new Response<CurrencyListDto>();
            try
            {
                var result = _currencyManager.UpdateCurrency(currencyDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<CurrencyListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CurrencyListDto>();
            try
            {
                var result = _currencyManager.DeleteCurrency(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<CurrencyListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<CurrencyListDto>();
            try
            {
                var result = _currencyManager.GetCurrency(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
