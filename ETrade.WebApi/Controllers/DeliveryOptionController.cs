...
namespace ETrade.WebApi.Controllers
{
    ...
    public class DeliveryOptionController : ControllerBase
    {
        ...
        private readonly IDeliveryOptionService _deliveryOptionManager;
        private readonly IAccountService _accountManager;


        private readonly List<RoleMethodListDto> _UserMethods;

        public DeliveryOptionController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _deliveryOptionManager = new DeliveryOptionManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<DeliveryOptionLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<DeliveryOptionFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<DeliveryOptionLoadMoreDto>();
            try
            {

                var result = _deliveryOptionManager.FilterDeliveryOptionList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.DeliveryOptionExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<DeliveryOptionListDto> Add([FromBody] DeliveryOptionDto deliveryOptionDto, [FromQuery] string Authorization)
        {
            var response = new Response<DeliveryOptionListDto>();
            try
            {
                var result = _deliveryOptionManager.AddDeliveryOption(deliveryOptionDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<DeliveryOptionListDto> Update([FromBody] DeliveryOptionDto deliveryOptionDto, [FromQuery] string Authorization)
        {
            var response = new Response<DeliveryOptionListDto>();
            try
            {
                var result = _deliveryOptionManager.UpdateDeliveryOption(deliveryOptionDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<DeliveryOptionListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<DeliveryOptionListDto>();
            try
            {
                var result = _deliveryOptionManager.DeleteDeliveryOption(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<DeliveryOptionListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<DeliveryOptionListDto>();
            try
            {
                var result = _deliveryOptionManager.GetDeliveryOption(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
