...
namespace ETrade.WebApi.Controllers
{
    ...
    public class GenderController : ControllerBase
    {
        ...
        public GenderController(IHttpContextAccessor httpContextAccessor, IAccountService accountManager)
        {
            ...
            _genderManager = new GenderManager(UserName, IpAddress);
            _accountManager = accountManager;
        }

        [HttpPost]
        [Route("GetAll")]
        public Response<GenderLoadMoreDto> GetAll([FromBody] BaseLoadMoreFilter<GenderFilter> filter, [FromQuery] string Authorization)
        {
            var response = new Response<GenderLoadMoreDto>();
            try
            {

                var result = _genderManager.FilterGenderList(filter);
            ...
            catch (Exception ex)
            {
                ...
                {
                    ErrorCode = ErrorMessageCode.GenderExceptionError,
                    Message = ex.Message
                });
            }
        ...
        [HttpPost]
        [Route("Add")]
        public Response<GenderListDto> Add([FromBody] GenderDto genderDto, [FromQuery] string Authorization)
        {
            var response = new Response<GenderListDto>();
            try
            {
                var result = _genderManager.AddGender(genderDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpPut]
        [Route("Update")]
        public Response<GenderListDto> Update([FromBody] GenderDto genderDto, [FromQuery] string Authorization)
        {
            var response = new Response<GenderListDto>();
            try
            {
                var result = _genderManager.UpdateGender(genderDto);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        public Response<GenderListDto> Delete(long id, [FromQuery] string Authorization)
        {
            var response = new Response<GenderListDto>();
            try
            {
                var result = _genderManager.DeleteGender(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
        ...
        [HttpGet]
        [Route("Get/{id:long}")]
        public Response<GenderListDto> Get(long id, [FromQuery] string Authorization)
        {
            var response = new Response<GenderListDto>();
            try
            {
                var result = _genderManager.GetGender(id);
                if (result.ErrorMessages.Count > 0)
                {
                    response.StatusCode = ResponseStatusCode.Error;
                    response.Message.AddRange(result.ErrorMessages);
